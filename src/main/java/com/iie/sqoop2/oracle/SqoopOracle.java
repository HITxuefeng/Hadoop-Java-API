...
public class SqoopOracle {

	public void sqoop(String ip) {
		System.out.println("进行sqoop操作----");
		String sqoopurl = "http://"+ip+":12000/sqoop/";
		SqoopClient client = new SqoopClient(sqoopurl);
		MConnection newCon = client.newConnection(4);
		...
		conForms.getStringInput("connection.connectionString").setValue("jdbc:mysql://10.199.33.13:3306/test");
		...
		connectorForm.getStringInput("table.tableName").setValue("test_ha");
		...
		frameworkForm.getEnumInput("output.outputFormat").setValue("CSV");// Other
																				// option:
																				// SEQUENCE_FILE
	...
	public static void main(String[] args) {
		String ip = "10.199.33.11";
		SqoopOracle so = new SqoopOracle();
		so.sqoop(ip);
	}
}
